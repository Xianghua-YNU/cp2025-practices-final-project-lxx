...
def find_matching_pairs(optical_dir, radio_dir, tol_arcsec=5):
    """通过坐标匹配光学和射电图像"""
    if not os.path.exists(optical_dir) or not os.path.exists(radio_dir):
        raise FileNotFoundError("Input directories do not exist")
    ...
    for o_file in optical_files:
        ...
        try:
            ...
            print(f"Error reading {o_file}: {str(e)}")
            continue
        ...
        for r_file in radio_files:
            ...
            try:
                ...
                print(f"Error reading {r_file}: {str(e)}")
                continue
